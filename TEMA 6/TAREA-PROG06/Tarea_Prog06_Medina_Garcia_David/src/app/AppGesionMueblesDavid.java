...
//                                                        salir2 = true;
    ...
    private static void recuperarFichero() {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(NOMBRE_ARCHIVO))) {
            ...
            if (listaMuebles.isEmpty()) {
                Utilidades.visualizarMensaje("No hay datos regsitrados.");
            } else {
        ...
        if (listaMuebles.isEmpty()) {
            ...
            do {
                ...
                if (idMueble == 0) {
                    ...
                    String buscar = "";
                    do {
                        Utilidades.visualizarMensaje("¿Quieres buscar otro mueble?(S/N)");
                        buscar = sn.next();
            ...
            do {
                try {
                    ...
                    if (idMueble == 0) {
                        ...
                        String confirmar = "";
                        do {
                            Utilidades.visualizarMensaje("¿Desea eliminar el mueble? (S/N)");
                            confirmar = sn.next();
                            if ("s".equalsIgnoreCase(confirmar)) {
                                ...
                                // Inicia un bloque try-catch para asegurar la correcta gestión de recursos
                                // Utilizamos ObjectOutputStream y FileOutputStream para guardar objetos en un archivo
                                try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(ficheroTemporal)); 
                                        ObjectInputStream ois = new ObjectInputStream(new FileInputStream(archivoActual))) {
    ...
    private static void eliminarFichero() throws IOException {
        ...
        if (archivo.exists()) {
            ...
            Utilidades.visualizarMensaje("Se va a eliminar el archivo " + NOMBRE_ARCHIVO + "\n¿Quieres continuar?(S/N)");
            String continuar = "";
            do {
                continuar = sn.next();
                if ("s".equalsIgnoreCase(continuar)) {
                    ...
                    Utilidades.visualizarMensaje("No se elimino el archivo.");
                } else {
                    Utilidades.visualizarMensaje("ERROR! Debe pulsar S/N para continuar.");
                }
                // Repite el bucle mientras la respuesta no se S/N
